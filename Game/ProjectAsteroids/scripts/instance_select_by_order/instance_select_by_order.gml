/// @description instance_select_by_order( obj_or_instance_or_array_of_them, condition_string, condition_value [, numb_add=0]);
/// @function instance_select_by_order
/// @param {array|instance|object} obj_or_instance_or_array_of_them - The instances, objects, or combination of them
/// @param {string} condition_string - The condition for testing. The posible values can be:
/// ```
/// ">"  "<"  ">="  "<="  "="  "=="  "!="  "<>"  "mod"  "%"  "mod!"  "%!"  "!mod"  "!%"
/// ```
/// @param {any} condition_value The value to compare against the instance order
/// @param {integer} [numb_add=0] - The number to add before comparing the instance order with the condition
...
function instance_select_by_order() {
	 ...
	 * 
	 * For more info about `obj_or_instance_or_array_of_them` argument check {@linkcode instance_select}
	 * 
	 * The internal process of selection is the following:
	 *     - First, all instances of obj_or_instance_or_array_of_them are 
	 *         selected and numbered from `0` to `n-1` (where `n` is the number of total instances)
	 *     - Then, each instance number is compared using the `condition_string` and the `condition_value`
	 *     - If the condition_string can be `"mod"`,  `"%"`,  `"mod!"`,  `"%!"`,  `"!mod"` or `"!%"` then the `numb_add` parameter is
	 ...
	 *
	 *     It is the same that saying all instances which `(n mod 2) != 0`
	 *     You can also use `"mod!"`, `"!%"`  or  `"%!"` for the same result. 
	 *         
	 *         
	 * Another example:
	 *     Suppose that you want to select obj_box the instances which 
	 *     number (sorted from left to right in the room) is 1, 4, 7, 10, etc.  
	 *     That is the same that all instances which `(n mod 3) + 1 == 0`
	 ...
	 *         arr = instance_select_by_order( arr, "mod", 3, 1);
	 *     
	 *             
	 * The last Example:
	 ...
	 *         arr = instance_select_by_order( arr, "<", 16); 
	 *
	 *     This will return an array with ALL the ids of the instances of the 
	 *     `obj_tree` which number is below 16. (remember that the numbers starts from zero)
	 *         
	 *         
	 *     
	 */
